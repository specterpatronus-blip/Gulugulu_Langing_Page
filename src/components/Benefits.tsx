...
const Benefits = () => {
  return (
    <section id="benefits" className="bg-bg py-16 sm:py-24 md:py-32">
        ...
        >
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-4 sm:mb-6">
              ...
              <span className="text-[10px] sm:text-xs text-muted uppercase tracking-[0.2em] sm:tracking-[0.3em]">Propuesta de Valor</span>
            </div>
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          {benefits.map((benefit, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 p-4 sm:p-4 bg-surface/30 hover:bg-surface border border-stroke rounded-2xl sm:rounded-full transition-all duration-500 cursor-pointer"
            >
              {/* Image thumbnail */}
              <div className="w-full sm:w-20 h-32 sm:h-20 rounded-xl sm:rounded-full overflow-hidden flex-shrink-0">
                <img src={benefit.image} alt={benefit.title} className="w-full h-full object-cover transition-all duration-500 scale-110 group-hover:scale-100 opacity-100" loading="lazy" />
              </div>
              
              <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 px-1 sm:px-4 w-full">
                <div className="flex flex-col">
                  <h3 className="text-base sm:text-lg md:text-xl font-medium text-text-primary group-hover:translate-x-2 transition-transform duration-500">
                    {benefit.title}
                  </h3>
                  <p className="text-xs sm:text-sm text-muted mt-1 max-w-md group-hover:translate-x-2 transition-transform duration-500 delay-75 line-clamp-2 sm:line-clamp-none">
                    {benefit.description}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4 sm:gap-6 text-xs text-muted">
                  <div className="flex items-center gap-1.5 text-text-primary">
                    ...
                    <span className="font-medium text-accent text-xs sm:text-sm whitespace-nowrap">{benefit.metric}</span>
                  </div>

                  {/* Arrow — mobile inline, desktop separate */}
                  <div className="sm:hidden w-8 h-8 flex items-center justify-center rounded-full bg-stroke/30 text-text-primary flex-shrink-0 ml-auto">
