...
const GuluMascot = () => {
  return (
    <section id="gulu" className="relative bg-bg py-24 md:py-32 overflow-hidden">
      {/* Background Decorative Elements */}
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
        <div className="absolute top-[10%] left-[5%] w-64 h-64 bg-accent/40 rounded-full blur-[100px] animate-pulse" />
        <div className="absolute bottom-[10%] right-[5%] w-96 h-96 bg-blue-400/30 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            {/* Main Gulu Card */}
            <div className="relative w-full max-w-md aspect-square bg-surface/40 backdrop-blur-xl border border-stroke rounded-[60px] flex items-center justify-center overflow-hidden group shadow-2xl">
          ...
          <div className="flex flex-col">
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs text-muted uppercase tracking-[0.3em]">El Corazón de Gulugulu</span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {traits.map((trait, i) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="flex flex-col p-6 bg-surface/20 border border-stroke rounded-3xl hover:border-accent/50 transition-colors group"
                >
                  ...
                  <h4 className="text-lg font-medium text-text-primary mb-1">{trait.title}</h4>
                  <p className="text-xs text-muted leading-relaxed">{trait.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-32 p-12 rounded-[40px] bg-surface border border-stroke relative overflow-hidden text-center"
        >
          ...
          <h3 className="text-2xl md:text-4xl font-display italic text-text-primary max-w-2xl mx-auto leading-tight">
            "En Gulugulu, aprender debería sentirse como explorar, no como depender."
          </h3>
          <p className="text-sm text-muted mt-6 uppercase tracking-widest">— Gulu, Tu Compañero de Aprendizaje</p>
        </motion.div>
      </div>
    </section>
  )
